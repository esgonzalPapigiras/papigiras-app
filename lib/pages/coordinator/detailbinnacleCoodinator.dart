...
class DetalleBitacoraCoordScreen extends StatefulWidget {
  ...
  const DetalleBitacoraCoordScreen({super.key, required this.idHito, required this.login});

  @override
  State<DetalleBitacoraCoordScreen> createState() => _DetalleBitacoraCoordScreenState();
}

class _DetalleBitacoraCoordScreenState extends State<DetalleBitacoraCoordScreen> {
  ...
  void initState() {
    super.initState();
    _hitoDetailFuture = _coordinatorProvider.getHitoComplete(widget.idHito, widget.login.tourSalesId.toString());
  }
            ...
            child: ElevatedButton.icon(
              onPressed: () {
                QuickAlert.show(
                  ...
                  title: 'Confirmación',
                  text: '¿Está seguro que quiere borrar el hito? Se perderá para siempre.',
                  confirmBtnText: 'Sí',
                  cancelBtnText: 'No',
              ...
              label: const Text('Eliminar Hito', style: TextStyle(color: Colors.white)),
              style: ElevatedButton.styleFrom(backgroundColor: Colors.red),
            ),
          ),
          const SizedBox(height: 20),
          Expanded(child: ListView(children: _buildBinnacleEntries(hitoDetail))),
        ],
      ),
    );
  }
            ...
            children: [
              Row(
                children: [
                  ...
                  Text(entry['time'].toString(), style: const TextStyle(color: Colors.teal, fontSize: 16, fontWeight: FontWeight.bold)),
                  const SizedBox(width: 8),
                  Expanded(child: Text(entry['activity'].toString(), style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600))),
                ],
              ),
              const SizedBox(height: 8),
              Text(entry['description'].toString(), style: const TextStyle(color: Colors.grey, fontSize: 14)),
